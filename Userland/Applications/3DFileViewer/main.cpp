...
ByteString const man_file = "/usr/share/man/man1/Applications/3DFileViewer.md";

NonnullRefPtr<GLContextWidget> setupGLWidget(NonnullRefPtr<GUI::Window> window)
{
...
void setupMenu(NonnullRefPtr<GUI::Application> app, const GUI::Icon& app_icon, NonnullRefPtr<GUI::Window> window, NonnullRefPtr<GLContextWidget> widget)
{
    {
...
ErrorOr<NonnullRefPtr<GUI::Window>> setupMainWindow(NonnullRefPtr<GUI::Application> app, StringView filename)
{
    ...
    auto widget = setupGLWidget(window);

    setupMenu(app, app_icon, window, widget);
    ...
    if (file.is_error()) {
        ...
        return Error::from_errno(file.error().code());
    }
...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto window = setupMainWindow(app, filename);
    if (window.is_error())
        return -1;
